...
def pd_control_all(servos, targets, duration, dt=0.05, Kp=1.0, Kd=0.1):
    """
    Uses PD control to drive each servo to its target following a cubic trajectory.
    ...
    # Initialize previous measured angles for derivative estimation.
    prev_angles = list(p0s)
    ...
    for step in range(steps + 1):
        ...
        for i, servo in enumerate(servos):
            p0 = p0s[i]
            pf = targets[i]
            # Compute desired position and velocity along the cubic trajectory.
            p_desired = cubic_trajectory(p0, pf, t, duration)
            v_desired = cubic_trajectory_velocity(p0, pf, t, duration)
            # Measure current position.
            p_actual = servo.get_physical_angle()
            # Estimate velocity via finite difference.
            v_actual = (p_actual - prev_angles[i]) / dt
            # Compute PD control output.
            error = p_desired - p_actual
            error_dot = v_desired - v_actual
            u = Kp * error + Kd * error_dot
            # Command the servo with an incremental correction.
            servo.move(p_actual + u)
            prev_angles[i] = p_actual
        time.sleep(dt)
    # Finally, ensure each servo is commanded to reach the target exactly.
    for i, servo in enumerate(servos):
...
async def my_task():
    ...
    pd_control_all([servo1, servo2, servo3, servo4, servo5],
    ...
    print("Moving to first forward bend (using PD control)...")
    pd_control_all([servo2, servo3, servo1, servo4, servo5],
    ...
    print("Moving to second forward bend (using PD control)...")
    pd_control_all([servo2, servo4, servo1, servo5, servo3],
