...
async def log_joint_angles(joint_1, joint_2, joint_3, joint_4, joint_5):
    ...
    while True:
        ...
        await asyncio.sleep(0.5)  # Adjust logging frequency (every 0.5 seconds)
...
async def my_task():
	...
	axs[0].plot(timestamps, joint_1, label='joint_1',color= 'k')
	axs[0].set_ylabel('Joint 1 (°)')
	axs[0].legend()
	axs[0].margins(x=0)
	axs[0].grid(True)
	axs[1].plot(timestamps, joint_2, label='joint_2',color= 'k')
	axs[1].set_ylabel('Joint 2 (°)')
	axs[1].legend()
	axs[1].grid(True)
	
	axs[2].plot(timestamps, joint_3, label='joint_3', color='k')
	axs[2].set_ylabel('Joint 3 (°)')
	axs[2].legend()
	axs[2].grid(True)
	
	axs[3].plot(timestamps, joint_4, label='joint_4',color= 'k')
	axs[3].set_ylabel('Joint 4 (°)')
	axs[3].legend()
	axs[3].grid(True)
	
	axs[4].plot(timestamps, joint_5, label='joint_5', color='k')
